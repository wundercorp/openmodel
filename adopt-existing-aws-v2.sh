...
for distribution in (data.get("DistributionList") or {}).get("Items") or []:
    ...
    if not any(domain_name.startswith(bucket_name + ".") for domain_name in origin_domains):
        print(
            "A CloudFront distribution already owns the alias, but its origin does not match "
...
PY
  ...
  if [[ "$cloudfront_discovery_status" == "3" ]]; then
    fail_adoption "No CloudFront distribution in this AWS account owns $OPENMODEL_WEB_HOSTNAME. The alias may belong to another AWS account or another AWS service. Do not retry apply until that ownership conflict is resolved."
  fi
...
aws lambda get-function \
  "${aws_arguments[@]}" \
  --function-name "$api_function_name" >/dev/null
...
api_id="$(
  python3 - "$apis_json" "$api_name" <<'PY'
import json
import sys

json_file, api_name = sys.argv[1:3]

with open(json_file, "r", encoding="utf-8") as file_handle:
    data = json.load(file_handle)

matches = [item for item in data.get("Items", []) if item.get("Name") == api_name]

if len(matches) != 1:
    print(f"Expected exactly one API named {api_name}, found {len(matches)}.", file=sys.stderr)
    sys.exit(1)

print(matches[0]["ApiId"])
PY
)" || fail_adoption "Could not safely identify the API Gateway HTTP API."
...
api_mapping_id="$(
  python3 - "$api_mappings_json" "$api_id" <<'PY'
import json
import sys

json_file, expected_api_id = sys.argv[1:3]
...
for mapping in data.get("Items", []):
    mapping_key = mapping.get("ApiMappingKey")
    if mapping_key not in (None, ""):
        continue
    root_mappings.append(mapping)
...
if mapping.get("ApiId") != expected_api_id:
    print(
        f"The root mapping points to API {mapping.get('ApiId')}, not expected API {expected_api_id}.",
        file=sys.stderr,
    )
    sys.exit(2)

print(mapping["ApiMappingId"])
PY
