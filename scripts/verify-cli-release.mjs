...
if (process.platform === 'win32') {
  fs.writeFileSync(builderStudioExecutablePath, `@echo telemetry enable>${builderStudioArgumentsFilePath}\r\n`, 'utf8');
} else {
  fs.writeFileSync(
    builderStudioExecutablePath,
    `#!/usr/bin/env node\nrequire('node:fs').writeFileSync(${JSON.stringify(builderStudioArgumentsFilePath)}, process.argv.slice(2).join(' '));\n`,
    { mode: 0o755 },
  );
}
...
if (fs.readFileSync(builderStudioArgumentsFilePath, 'utf8').trim() !== 'telemetry enable') {
  fail('om setup bs did not invoke bs telemetry enable.');
}
